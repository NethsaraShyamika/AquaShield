...
export const createReport = async (req, res) => {
  try {
    const { incidentType, description, latitude, longitude, placeName, incidentDate, speciesInvolved } = req.body;
    ...
    const report = await Report.create({
      ...
      evidence,
      speciesInvolved: speciesInvolved ? JSON.parse(speciesInvolved) : [],
    });
...
export const getMyReportById = async (req, res) => {
  try {
    const report = await Report.findOne({ _id: req.params.id, reportedBy: req.user._id })
      .populate("speciesInvolved", "name commonName");
...
// ─── USER: Update own report (only if Pending) ───────────────────────────────
export const updateMyReport = async (req, res) => {
  try {
    ...
    if (speciesInvolved) report.speciesInvolved = JSON.parse(speciesInvolved);

    // Append new evidence if uploaded
...
// ─── USER: Delete own report (only if Pending) ───────────────────────────────
export const deleteMyReport = async (req, res) => {
  try {
    ...
    if (!report) return res.status(404).json({ message: "Report not found." });
    if (report.status !== "Pending") {
      return res.status(403).json({ message: "Only Pending reports can be deleted." });
    }
...
export const updateReportStatus = async (req, res) => {
  try {
    ...
    const validStatuses = ["Pending", "Under Review", "Verified", "Dismissed", "Resolved"];
