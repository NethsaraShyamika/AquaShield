  ...
  return (
    <div>
      ...
      <div style={{ position: "relative" }}>
        ...
        {evs.map((ev, i) => {
          const Icon = ev.icon; const isLast = i === evs.length - 1; return (
...
function CreateModal({ onClose, onCreate, saving }) {
  ...
  // Update a top-level field and clear its error immediately
  const set = (k, v) => { setForm(f => ({ ...f, [k]: v })); if (submitted) setErrors(e => ({ ...e, [k]: "" })) };
  // Update a legalAction sub-field and clear its error
  ...
  const validate = () => {
    const e = {};
    // Case Number: required, must follow CASE-XXX pattern
    ...
    // Assigned Officer: required, letters only
    ...
    // Notes: optional but if provided must be at least 10 chars
    if (form.notes.trim() && form.notes.trim().length < 10)
      e.notes = "Notes must be at least 10 characters if provided.";
    // Legal fields: validate only when any legal field is filled
    ...
    if (anyLegal) {
      ...
      if (!la.courtDate) e.legal_courtDate = "Court date is required when legal action is filled.";
        ...
        if (new Date(la.courtDate) < today) e.legal_courtDate = "Court date cannot be in the past.";
      }
  ...
  // Handle form submit: run validation, show errors or call onCreate
  ...
  const fieldStyle = (key) => ({ ...iS, border: `1px solid ${errors[key] ? "#f87171" : P.border}` });
      ...
      <div style={{ width: "100%", maxWidth: 500, borderRadius: 20, border: `1px solid ${P.border}`, overflow: "hidden", background: P.cardBg, boxShadow: "0 30px 80px rgba(0,0,0,0.35)" }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: "16px 22px", display: "flex", flexDirection: "column", gap: 14, maxHeight: "68vh", overflowY: "auto" }}>
            ...
            {/* Validation summary banner — shown only after first submit attempt */}
            {submitted && errCount > 0 && (
              <div style={{ background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.28)", borderRadius: 12, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <p style={{ margin: 0, fontSize: 12, color: "#f87171", fontWeight: 600 }}>
                  Please fix {errCount} {errCount === 1 ? "error" : "errors"} before submitting.
                </p>
              </div>
            )}
            ...
            {/* ── Case Number ── */}
            <div>
              <label style={lS}>Case Number <span style={{ color: "#f87171" }}>*</span></label>
              <input
                value={form.caseNumber}
                onChange={e => set("caseNumber", e.target.value.toUpperCase())}
                placeholder="CASE-001"
                maxLength={20}
                style={{ ...fieldStyle("caseNumber"), fontFamily: "monospace", letterSpacing: 2, color: P.cyanLight }}
              />
              {errors.caseNumber && <p style={{ margin: "5px 0 0", fontSize: 11, color: "#f87171" }}>{errors.caseNumber}</p>}
            </div>
            ...
            <div>
              ...
              {rLoading
                ...
                : <select
                  ...
                  style={{ ...fieldStyle("reportId"), cursor: "pointer" }}
              ...
              {errors.reportId && <p style={{ margin: "5px 0 0", fontSize: 11, color: "#f87171" }}>{errors.reportId}</p>}
            </div>
            ...
            <div>
              <label style={lS}>
                <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <Lock size={10} style={{ color: P.muted }} /> Location
                  <span style={{ fontWeight: 400, fontSize: 11, color: P.muted2 }}>— auto-filled from GPS</span>
                </span>
              </label>
            ...
            <div>
              ...
              <input
                ...
                onChange={e => set("assignedOfficer", e.target.value)}
                placeholder="Officer full name…"
                style={fieldStyle("assignedOfficer")}
              />
              {errors.assignedOfficer && <p style={{ margin: "5px 0 0", fontSize: 11, color: "#f87171" }}>{errors.assignedOfficer}</p>}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Add relevant case notes…"
                style={{ ...fieldStyle("notes"), resize: "none" }}
              />
              {errors.notes && <p style={{ margin: "5px 0 0", fontSize: 11, color: "#f87171" }}>{errors.notes}</p>}
            </div>

            {/* ── Legal Action (optional section) ── */}
            <div style={{ background: "rgba(167,139,250,0.05)", border: "1px solid rgba(167,139,250,0.15)", borderRadius: 12, padding: 14 }}>
              ...
              <p style={{ margin: "0 0 12px", fontSize: 11, color: P.muted2 }}>Optional — but if any field is filled, Court Name and Court Date become required.</p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {/* Court Name */}
                <div>
                  <label style={{ ...lS, fontSize: 11 }}>Court Name</label>
                  <input type="text" value={form.legalAction.courtName} onChange={e => setLegal("courtName", e.target.value)} placeholder="e.g. Colombo High Court"
                    style={{ ...iS, padding: "8px 10px", fontSize: 13, border: `1px solid ${errors.legal_courtName ? "#f87171" : P.border}` }} />
                  {errors.legal_courtName && <p style={{ margin: "4px 0 0", fontSize: 10, color: "#f87171" }}>{errors.legal_courtName}</p>}
                </div>
                {/* Court Date */}
                <div>
                  <label style={{ ...lS, fontSize: 11 }}>Court Date</label>
                  <input type="date" value={form.legalAction.courtDate} onChange={e => setLegal("courtDate", e.target.value)}
                    style={{ ...iS, padding: "8px 10px", fontSize: 13, border: `1px solid ${errors.legal_courtDate ? "#f87171" : P.border}` }} />
                  {errors.legal_courtDate && <p style={{ margin: "4px 0 0", fontSize: 10, color: "#f87171" }}>{errors.legal_courtDate}</p>}
                </div>
                {/* Fine */}
                <div>
                  <label style={{ ...lS, fontSize: 11 }}>Fine (LKR)</label>
                  <input type="number" min="0" value={form.legalAction.fineAmount} onChange={e => setLegal("fineAmount", e.target.value)} placeholder="e.g. 50000"
                    style={{ ...iS, padding: "8px 10px", fontSize: 13, border: `1px solid ${errors.legal_fineAmount ? "#f87171" : P.border}` }} />
                  {errors.legal_fineAmount && <p style={{ margin: "4px 0 0", fontSize: 10, color: "#f87171" }}>{errors.legal_fineAmount}</p>}
                </div>
                {/* Jail Duration */}
                <div>
                  <label style={{ ...lS, fontSize: 11 }}>Jail Duration</label>
                  <input type="text" value={form.legalAction.jailDuration} onChange={e => setLegal("jailDuration", e.target.value)} placeholder="e.g. 6 months"
...
function EditModal({ c, onClose, onSave, saving }) {
  ...
  const [errors, setErrors] = useState({});
  // Track whether the user has submitted at least once
  const [submitted, setSubmitted] = useState(false);

  const set = (k, v) => { setForm(f => ({ ...f, [k]: v })); if (submitted) setErrors(e => ({ ...e, [k]: "" })); };
  // Update legalAction sub-field and clear its error on change
  const setLegal = (k, v) => { setForm(f => ({ ...f, legalAction: { ...f.legalAction, [k]: v } })); if (submitted) setErrors(e => ({ ...e, [`legal_${k}`]: "" })); };

  // Validate all editable fields; returns errors object (empty = valid)
  const validate = () => {
    const e = {};
    // Officer: required, min 3 chars
    if (!form.assignedOfficer.trim())
      e.assignedOfficer = "Assigned officer name is required.";
    else if (form.assignedOfficer.trim().length < 3)
      e.assignedOfficer = "Officer name must be at least 3 characters.";
    // Notes: optional but min 10 chars if provided
    if (form.notes.trim() && form.notes.trim().length < 10)
      e.notes = "Notes must be at least 10 characters if provided.";
    // Legal fields: all conditional on any field being filled
    const la = form.legalAction;
    const anyLegal = la.courtName || la.courtDate || la.fineAmount || la.jailDuration;
    if (anyLegal) {
      if (!la.courtName.trim()) e.legal_courtName = "Court name is required when legal action is filled.";
  ...
  // Handle Save: validate first, then call onSave
  const handleSave = () => {
    setSubmitted(true);
    const errs = validate();
    if (Object.keys(errs).length > 0) { setErrors(errs); return; }
  ...
  const errCount = Object.keys(errors).filter(k => errors[k]).length;
  // Helper: turn border red when a field has an error
  const fieldStyle = (key) => ({ ...iS, border: `1px solid ${errors[key] ? "#f87171" : P.border}` });
        ...
        <div style={{ padding: "16px 22px", display: "flex", flexDirection: "column", gap: 14, maxHeight: "68vh", overflowY: "auto" }}>
          ...
          {submitted && errCount > 0 && (
            <div style={{ background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.28)", borderRadius: 12, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
              <AlertTriangle size={15} style={{ color: "#f87171", flexShrink: 0 }} />
              <p style={{ margin: 0, fontSize: 12, color: "#f87171", fontWeight: 600 }}>
                Please fix {errCount} {errCount === 1 ? "error" : "errors"} before saving.
              </p>
            </div>
          )}
          ...
          <div>
            <label style={lS}>Assigned Officer <span style={{ color: "#f87171" }}>*</span></label>
            <input value={form.assignedOfficer} onChange={e => set("assignedOfficer", e.target.value)} placeholder="Officer name…" style={fieldStyle("assignedOfficer")} />
            {errors.assignedOfficer && <p style={{ margin: "5px 0 0", fontSize: 11, color: "#f87171" }}>{errors.assignedOfficer}</p>}
          </div>
          ...
          <div>
            <label style={lS}>Notes <span style={{ fontWeight: 400, color: P.muted2, fontSize: 11 }}>(optional — min 10 chars if provided)</span></label>
            <textarea value={form.notes} onChange={e => set("notes", e.target.value)} rows={3} placeholder="Case notes…" style={{ ...fieldStyle("notes"), resize: "none" }} />
            {errors.notes && <p style={{ margin: "5px 0 0", fontSize: 11, color: "#f87171" }}>{errors.notes}</p>}
          </div>
          ...
          <div style={{ background: "rgba(167,139,250,0.05)", border: "1px solid rgba(167,139,250,0.15)", borderRadius: 12, padding: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <div>
                <label style={{ ...lS, fontSize: 11 }}>Court Name</label>
                <input type="text" value={form.legalAction.courtName} onChange={e => setLegal("courtName", e.target.value)} placeholder="Court name…"
                  style={{ ...iS, padding: "8px 10px", fontSize: 13, border: `1px solid ${errors.legal_courtName ? "#f87171" : P.border}` }} />
                {errors.legal_courtName && <p style={{ margin: "4px 0 0", fontSize: 10, color: "#f87171" }}>{errors.legal_courtName}</p>}
              </div>
              {/* Court Date */}
              ...
              <div>
                <label style={{ ...lS, fontSize: 11 }}>Fine (LKR)</label>
                <input type="number" min="0" value={form.legalAction.fineAmount} onChange={e => setLegal("fineAmount", e.target.value)} placeholder="0"
                  style={{ ...iS, padding: "8px 10px", fontSize: 13, border: `1px solid ${errors.legal_fineAmount ? "#f87171" : P.border}` }} />
                {errors.legal_fineAmount && <p style={{ margin: "4px 0 0", fontSize: 10, color: "#f87171" }}>{errors.legal_fineAmount}</p>}
              </div>
              {/* Jail Duration */}
