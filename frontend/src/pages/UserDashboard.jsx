...
const TAIL_SHAPE_REFERENCES = {
  Rounded: {
    description: "Rounded tail shape with a smooth, curved edge.",
    image: "/rounded-tail-reference.png",
  },
  Crescent: {
    description: "Crescent tail shape with a curved, moon-like edge.",
    image: "/crescent-tail-reference.png",
  },
  Forked: {
    description: "Forked tail shape with a split end.",
    image: "/forked-tail-reference.png",
  },
  Square: {
    description: "Square tail shape with a straight, flat end.",
    image: "/square-tail-reference.png",
  },
  Pointed: {
    description: "Pointed tail shape with a narrow, sharp end.",
    image: "/pointed-tail-reference.png",
  },
};
...
export default function UserDashboard() {
  ...
  useEffect(() => {
    if (identifyMatches.length === 0) {
      setSelectedIdentifySpeciesId("");
      return;
    }
    ...
    if (!hasSelection) {
      setSelectedIdentifySpeciesId(toSpeciesKey(identifyMatches[0]));
    }
  }, [identifyMatches, selectedIdentifySpeciesId]);
  ...
  const identifySuggestions = useMemo(() => {
    ...
    const byName = speciesCatalog.filter((item) => {
      const commonName = (item.name || "").toLowerCase();
      const scientificName = (item.scientificName || "").toLowerCase();
      return commonName.includes(query) || scientificName.includes(query);
    });
        ...
        <section className="identify-shell card-reveal delay-2">
            ...
            >
              {identifySuggestions.map((item) => (
                <button
                  ...
                  type="button"
                  className="identify-suggestion-item"
                  onClick={() => {
                    setIdentifyQuery(item.name || "");
          ...
          <div className="identify-filters">
            ...
            {identifyBodyShape && BODY_SHAPE_REFERENCES[identifyBodyShape] ? (
              <div className="body-shape-reference">
                <img
                  src={BODY_SHAPE_REFERENCES[identifyBodyShape].image}
                  alt={`${identifyBodyShape} body shape reference`}
                  className="body-shape-reference-image"
                />
                <p className="body-shape-reference-text">
                  {BODY_SHAPE_REFERENCES[identifyBodyShape].description}
                </p>
              </div>
            ) : null}
          </div>
          ...
          <div className="identify-filters">
            ...
            <div className="pills">
              <button
                type="button"
                className={`pill ${identifyTailShape === "" ? "pill-active" : ""}`}
                onClick={() => setIdentifyTailShape("")}
              >
                All
              </button>
              {TAIL_SHAPES.map((shape) => (
                <button
                  key={`identify-tail-${shape}`}
                  type="button"
                  className={`pill ${identifyTailShape === shape ? "pill-active" : ""}`}
                  onClick={() => setIdentifyTailShape(shape)}
                >
                  {toTitleCase(shape)}
                </button>
              ))}
            ...
            TAIL_SHAPE_REFERENCES.Rounded ? (
              <div className="body-shape-reference">
                <img
                  ...
                  alt="Rounded tail shape reference"
                  className="body-shape-reference-image"
                />
                <p className="body-shape-reference-text">
                  {TAIL_SHAPE_REFERENCES.Rounded.description}
                </p>
              </div>
            ) : null}
            ...
            TAIL_SHAPE_REFERENCES.Crescent ? (
              <div className="body-shape-reference">
                <img
                  ...
                  alt="Crescent tail shape reference"
                  className="body-shape-reference-image"
                />
                <p className="body-shape-reference-text">
                  {TAIL_SHAPE_REFERENCES.Crescent.description}
                </p>
              </div>
            ) : null}
            {identifyTailShape === "forked" && TAIL_SHAPE_REFERENCES.Forked ? (
              <div className="body-shape-reference">
                <img
                  src={TAIL_SHAPE_REFERENCES.Forked.image}
                  alt="Forked tail shape reference"
                  className="body-shape-reference-image"
                />
                <p className="body-shape-reference-text">
                  {TAIL_SHAPE_REFERENCES.Forked.description}
                </p>
              </div>
            ) : null}
            {identifyTailShape === "square" && TAIL_SHAPE_REFERENCES.Square ? (
              <div className="body-shape-reference">
                <img
                  src={TAIL_SHAPE_REFERENCES.Square.image}
                  alt="Square tail shape reference"
                  className="body-shape-reference-image"
                />
                <p className="body-shape-reference-text">
                  {TAIL_SHAPE_REFERENCES.Square.description}
                </p>
              </div>
            ) : null}
            ...
            TAIL_SHAPE_REFERENCES.Pointed ? (
              <div className="body-shape-reference">
                <img
                  ...
                  alt="Pointed tail shape reference"
                  className="body-shape-reference-image"
                />
                <p className="body-shape-reference-text">
                  {TAIL_SHAPE_REFERENCES.Pointed.description}
                </p>
              </div>
            ) : null}
          </div>
          ...
          {identifyMatches.length === 0 ? (
            <p className="empty-state">
              No species found. Try another name, body shape, or tail shape.
            </p>
          ) : (
            <>
          ...
          {selectedIdentifySpecies ? (
            <article className="identify-detail-card">
              {resolveSpeciesImageUrl(selectedIdentifySpecies) &&
              !brokenIdentifyImages[toSpeciesKey(selectedIdentifySpecies)] ? (
                <img
                  src={resolveSpeciesImageUrl(selectedIdentifySpecies)}
                  alt={
                    selectedIdentifySpecies.name
                      ? `${selectedIdentifySpecies.name} full reference`
                      : "Selected species"
                  }
              ...
              <div className="section-head">
                ...
                <span
                  className={`chip ${selectedIdentifySpecies.isFullyBanned ? "chip-red" : "chip-amber"}`}
                >
              ...
              <div className="identify-detail-grid">
                <p>
                  ...
                  {formatBodyShapeLabel(selectedIdentifySpecies.bodyShape)}
                </p>
                <p>
                  <strong>Tail Shape:</strong>{" "}
                  {selectedIdentifySpecies.tailShape
                    ? toTitleCase(selectedIdentifySpecies.tailShape)
                    : "Unknown"}
                </p>
                <p>
                ...
                <p>
                  ...
                  {selectedIdentifySpecies.legalMinSizeCm
                    ? `${selectedIdentifySpecies.legalMinSizeCm} cm`
                    : "Not specified"}
                </p>
                <p>
